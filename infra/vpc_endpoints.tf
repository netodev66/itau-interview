...
resource "aws_vpc_endpoint" "sts" {
  ...
  tags = { Name = "${local.name_prefix}-endpoint-sts" }
}

# ─── Gateway Endpoints (free, route-table based) ──────────────────────────────

# ECR pulls image layers from S3 — gateway endpoint avoids NAT traffic costs
...
# DynamoDB gateway endpoint keeps app ↔ DynamoDB traffic inside AWS backbone
